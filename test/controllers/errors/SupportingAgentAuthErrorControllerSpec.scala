...
class SupportingAgentAuthErrorControllerSpec extends ControllerUnitTest {
  ...
  lazy val underTest = new SupportingAgentAuthErrorController(stubMessagesControllerComponents(), mockAppConfig, pageView)
  ...
  "The show method" should {
    "return an UNAUTHORIZED response when .show() is called" in {
      ...
      contentAsString(result) shouldBe pageView()(fakeRequest, stubMessages(), mockAppConfig).toString
    }
  }
}
