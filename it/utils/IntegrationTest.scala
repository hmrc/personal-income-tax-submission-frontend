...
trait IntegrationTest extends AnyWordSpecLike with Matchers with GuiceOneServerPerSuite with WireMockHelper
  ...
  def config(tailoring: Boolean = false, interestTailoring: Boolean = false, dividendsTailoring: Boolean = false): Map[String, Any] = commonConfig ++ Map(
  ...
  lazy val appWithTailoring: Application = new GuiceApplicationBuilder()
    .in(Environment.simple(mode = Mode.Dev))
    .configure(config(tailoring = true, interestTailoring = true, dividendsTailoring = true))
    .build()
