...
class UntaxedInterestAmountControllerISpec extends IntegrationTest with ViewHelpers {
  ...
  val untaxedAmountPageTitle = "Add an account with untaxed UK interest"
  val changeAmountPageTitle = "How much untaxed UK interest did you get?"

  lazy val id: String = UUID.randomUUID().toString

  def url(newId: String): String = s"$startUrl/$taxYear/interest/add-untaxed-uk-interest-account/$newId"
    ...
    "return an action" when {
      ...
      "there is no CYA data in session" which {
        lazy val result = {
          ...
          await(wsClient.url(s"$startUrl/$taxYear/interest/add-untaxed-uk-interest-account/UntaxedId")
