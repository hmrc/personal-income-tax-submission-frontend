...
class YouNeedAgentServicesControllerISpec extends IntegrationTest with ViewHelpers {
  ...
  val url = s"$startUrl/error/you-need-agent-services-account"
  ...
  val userScenarios: Seq[UserScenario[ExpectedResultsAllUsers, ExpectedResultsAllUsers]] = {
    Seq(UserScenario(isWelsh = false, isAgent = false, None, CommonExpectedEN),
      UserScenario(isWelsh = true, isAgent = false, None, CommonExpectedCY))
  }
