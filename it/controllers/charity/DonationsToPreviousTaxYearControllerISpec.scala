...
class DonationsToPreviousTaxYearControllerISpec extends IntegrationTest with ViewHelpers {
  ...
  def url(overrideYear: Int = 2022): String = s"$appUrl/$taxYear/charity/donations-after-5-april-$overrideYear"
      ...
      s"language is ${welshTest(user.isWelsh)} and request is from an ${agentTest(user.isAgent)}" should {
        ...
        "redirect to a correct URL when years don't match up" which {
          lazy val result: WSResponse = {
            ...
            urlGet(url(taxYear + 1), welsh = user.isWelsh, headers = Seq(HeaderNames.COOKIE -> playSessionCookies(taxYear)))
          }
          ...
          "has an SEE_OTHER status" in {
            ...
            result.headers("Location") shouldBe url()
          }
        }
      }
    }
