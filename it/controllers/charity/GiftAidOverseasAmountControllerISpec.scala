...
class GiftAidOverseasAmountControllerISpec extends IntegrationTest with ViewHelpers with GiftAidDatabaseHelper {
  ...
  val overseasDonatedAmountUrl = s"http://localhost:$port/income-through-software/return/personal-income/$taxYear/charity/amount-donated-to-overseas-charities"
  ...
  def getResult(cyaData: Option[GiftAidCYAModel],
    ...
    if(isAgent){
      authoriseAgent()
      await(wsClient.url(overseasDonatedAmountUrl).withHttpHeaders(
        HeaderNames.COOKIE -> agentSessionCookie, langHeader, xSessionId, csrfContent)
        .withFollowRedirects(false).get
      )
    } else {
      authoriseIndividual()
      await(wsClient.url(overseasDonatedAmountUrl).withHttpHeaders(langHeader, xSessionId, csrfContent)
        .withFollowRedirects(false).get()
      )
    }
  ...
  "as an individual" when {
    ...
    s"Calling GET $overseasDonatedAmountUrl" when {
      ...
      "there is cya data, but 'overseasDonationsViaGiftAid' has not been stored" should {
        ...
        lazy val result: WSResponse = getResult(Some(GiftAidCYAModel(oneOffDonationsViaGiftAid = Some(false))), None, isAgent = false)
      ...
      "'overseasDonationsViaGiftAid' is true - with the correct english content" should {
      ...
      "'overseasDonationsViaGiftAid' is true - with the correct welsh content" should {
        lazy val result: WSResponse = getResult(requiredSessionData, None, isAgent = false, welsh = true)
        ...
        h1Check(expectedH1Cy + " " + expectedCaption)
    ...
    s"Calling POST $overseasDonatedAmountUrl" when {
      ...
      "there is data stored" when {
        ...
        "the user has not entered an amount - with the correct welsh content" which {
        ...
        "the user has entered an amount too large - with the correct welsh content" which {
        ...
        "the user has entered an invalid amount - with the correct welsh content" which {
          lazy val result: WSResponse = postResult(requiredSessionData, None, isAgent = false, Map("amount" -> "|"), welsh = true)
        ...
        "the user has entered a valid amount" when {
          ...
          "this completes the cya model" should {
            lazy val result: WSResponse =
              postResult(Some(completeGiftAidCYAModel.copy(overseasDonationsViaGiftAidAmount = None)), None, isAgent = false, Map("amount" -> s"$validAmount"))
  ...
  "as an agent" when {
    ...
    s"Calling GET $overseasDonatedAmountUrl" when {
      ...
      "there is cya data, but 'overseasDonationsViaGiftAid' has not been stored" should {
        ...
        lazy val result: WSResponse = getResult(Some(GiftAidCYAModel(oneOffDonationsViaGiftAid = Some(false))), None, isAgent = true)
    ...
    s"Calling POST $overseasDonatedAmountUrl" when {
      ...
      "there is data stored" when {
        ...
        "the user has entered a valid amount" when {
          ...
          "this completes the cya model" should {
            lazy val result: WSResponse =
              postResult(Some(completeGiftAidCYAModel.copy(overseasDonationsViaGiftAidAmount = None)), None, isAgent = true, Map("amount" -> s"$validAmount"))
