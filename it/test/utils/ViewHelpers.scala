    ...
    for(i <- 1 to itemList.length){
      s"display a task list row for entry number $i" which {
        s"displays the correct name for entry number $i" in {
          document().select(s"#main-content > div > div > div.govuk-form-group > dl > div:nth-child($i) > dt")
            .text() shouldBe itemList(i-1)._1
        }
        s"displays the change link and has the correct hidden-change-text for entry number $i" in {
          document().select(s"#main-content > div > div > div.govuk-form-group > dl > div:nth-child($i) > dd > ul > li:nth-child($i) > a > span:nth-child($i)").text() shouldBe change
          document().select(s"#main-content > div > div > div.govuk-form-group > dl > div:nth-child($i) > dd > ul > li:nth-child($i) > a > span.govuk-visually-hidden")
            .text() shouldBe itemList(i-1)._2
        }
        s"displays the remove link and has the correct hidden-remove-text for entry number $i" in {
          document().select(s"#main-content > div > div > div.govuk-form-group > dl > div:nth-child($i) > dd > ul > li:nth-child(2) > a > span:nth-child($i)").text() shouldBe remove
          document().select(s"#main-content > div > div > div.govuk-form-group > dl > div:nth-child($i) > dd > ul > li:nth-child(2) > a > span.govuk-visually-hidden")
    ...
    s"has a link to change the language to $otherLanguage" which {
      ...
      s"has a link to change the language" in {
        document().select(".hmrc-language-select__list-item > a").attr("href") shouldBe
          s"/update-and-submit-income-tax-return/personal-income/language/${linkLanguage(otherLanguage).toLowerCase}"
      }
    }
  }
