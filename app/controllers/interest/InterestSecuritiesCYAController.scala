...
import models.interest.InterestSecuritiesCYAModel
import play.api.Logging
import play.api.i18n.I18nSupport
import play.api.mvc._
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendController
import utils.SessionHelper
import views.html.interest.InterestSecuritiesCYAView
...
class InterestSecuritiesCYAController @Inject()(interestSecuritiesCYAView: InterestSecuritiesCYAView,
  ...
  def show(taxYear: Int): Action[AnyContent] = commonPredicates(taxYear, INTEREST).async { implicit user =>
    Future.successful(Ok(interestSecuritiesCYAView(InterestSecuritiesCYAModel(Some(true), Some(1234), Some(true), Some(324)), taxYear)))
  }
