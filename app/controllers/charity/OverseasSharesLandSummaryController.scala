...
class OverseasSharesLandSummaryController @Inject()(overseasSharesLandSummaryView: OverseasSharesLandSummaryView)(
  ...
  def getOverseasCharities: List[String] = List("overseasCharity1")

  def show(taxYear: Int): Action[AnyContent] = commonPredicates(taxYear, GIFT_AID).apply { implicit user =>
    Ok(overseasSharesLandSummaryView(yesNoForm, taxYear, getOverseasCharities))
  }
    ...
    yesNoForm.bindFromRequest().fold(
      {
        formWithErrors =>
          BadRequest(
            overseasSharesLandSummaryView(formWithErrors, taxYear, getOverseasCharities)
          )
      },
      {
        yesNoForm => Redirect(controllers.charity.routes.OverseasSharesLandSummaryController.show(taxYear))
      }
    )
  }
