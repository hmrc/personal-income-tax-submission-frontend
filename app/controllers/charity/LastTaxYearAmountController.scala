...
@Singleton
class LastTaxYearAmountController @Inject()(view: LastTaxYearAmountView)(
  ...
  def submit(taxYear: Int): Action[AnyContent] = (authAction andThen journeyFilterAction(taxYear, GIFT_AID)) { implicit user =>
    ...
    form(user.isAgent, taxYear).bindFromRequest().fold(
      ...
      {
        submittedAmount => Redirect(controllers.charity.routes.LastTaxYearAmountController.show(taxYear))
      }
    )
