...
import views.html.dividends.CheckCloseCompanyLoanAmountView

import javax.inject.{Inject, Singleton}
import scala.concurrent.{ExecutionContext, Future}

@Singleton
class CheckCloseCompanyLoanAmountController @Inject()(authorisedAction: AuthorisedAction,
                                                      view: CheckCloseCompanyLoanAmountView,
  ...
  private def getStockDividendsCya(taxYear: Int,
    ...
    StockDividendsCheckYourAnswersModel.getCyaModel(cya, prior) match {
      ...
      case _ =>
        logger.info("[CheckCloseCompanyLoanAmountController][show] No CYA data in session. Redirecting to the task list.")
  ...
  private[controllers] def performSubmission(taxYear: Int,
    ...
    (cyaData match {
      ...
      case _ =>
        logger.info("[CheckCloseCompanyLoanAmountController][submit] CYA data or NINO missing from session.")
