...
import views.html.dividends.CheckRedeemableSharesAmountView

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class CheckRedeemableSharesAmountController @Inject()(authorisedAction: AuthorisedAction,
  ...
  private def getRedeemableShares(taxYear: Int)(implicit request: User[AnyContent]): Future[Result] = {
    stockDividendsSession.getAndHandle(taxYear)(errorHandler.futureInternalServerError()) { (cya, stockDividendsPrior) =>
      if (stockDividendsPrior.isDefined) {
        ...
        getRedeemableSharesCya(taxYear, cya, None)
      }
    }
  }
  ...
  private[controllers] def performSubmission(taxYear: Int,
    ...
    (cyaData match {
      ...
      case _ =>
        logger.info("[CheckRedeemableSharesAmountController][submit] CYA data or NINO missing from session.")
